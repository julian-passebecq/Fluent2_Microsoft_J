...
function variantsFor(id:typeof lessonMetadata[number]['id']):LessonVariant[]{
  switch(id){
    ...
    case 'group':return (['baseline','null-amounts'] as const).map(variant=>({id:variant,steps:groupingFrames(variant),validate:()=>validateGrouping(variant)}));
  }
}
export function validateCatalog(lessons=catalog){
  ...
  for(const lesson of lessons){
    ...
    for(const variant of lesson.variants){
      ...
      variant.validate();frames+=variant.steps.length;variants++;
    }
